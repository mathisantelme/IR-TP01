...
5. Est-ce que la solution actuelle est synchrone ou asynchrone ? Comment feriezvous pour implémenter l’autre solution ? Faites-le.

Actuellement cette solution est *asynchrone*. En effet les noeuds ne s'attendent pas mutuellement afin de "s'activer" en meme temps. Si on veut synchroniser cette solution, il nous créer un `chan` utilisé par le main afin de synchroniser les noeuds entre eux. Les noeuds quand à eux attendent le message du main qui leur permet de recevoir et envoyer des messages. Voici un extrait du code:
...
On modifie du code aussi dans la fonction `Run`. En effet on change la structure de la boucle `for` qui tourne idéfiniement et qui attend le message de synchronisation avant de recevoir un message.
...
func (r *RingNode) Run(sync chan int) {
	...
	for {
		...
		receivedMessage := <-r.inChannel // on réceptionne le message
		fmt.Println("From", r.localId, ": received", receivedMessage.id, receivedMessage.isLeader)
